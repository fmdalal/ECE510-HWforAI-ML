    ...
    generate
        for (gi = 0; gi < M; gi++) begin : row_gen
            for (gj = 0; gj < N; gj++) begin : col_gen
                ...
                bf16_mac pe_inst (
                    .clk_core(clk_core), .rst_n(rst_n & ~clear),
                    .en(en), .flush(flush),
                    .a(a_reg[gi][gj < K ? gj : K-1]),
                    .b(b_col[gj][gi < K ? gi : K-1]),
                    .acc_fp32_in (acc_fp32[gi][gj]),
                    .acc_fp32_out(fp32_out_w),
                    .acc_bf16_out(c_out[gi][gj])
                );
                always_ff @(posedge clk_core or negedge rst_n) begin : acc_fb_ff
                    if (!rst_n || clear) acc_fp32[gi][gj] <= 32'h0;
